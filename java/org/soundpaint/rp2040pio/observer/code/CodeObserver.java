 ...
 */
public class CodeObserver extends JFrame
{
  ...
  {
    console.println("Code Observer App");
  ...
  private int run()
  {
    final int refresh = options.getValue(optRefresh);
    try {
      getContentPane().add(new CodeViewPanel(console, sdk, refresh));
  ...
  {
    new CodeObserver(System.out, argv).run();
  }
}
